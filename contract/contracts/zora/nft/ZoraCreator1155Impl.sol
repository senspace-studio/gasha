    ...
    ERC1155DelegationStorageV1
{
    ...
    function _requireCanMintQuantity(
        uint256 tokenId,
        uint256 quantity
    ) internal view {
        TokenData storage tokenInformation = tokens[tokenId];
        if (
            tokenInformation.totalMinted + quantity > tokenInformation.maxSupply
        ) {
            revert CannotMintMoreTokens(
                tokenId,
                quantity,
                tokenInformation.totalMinted,
                tokenInformation.maxSupply
            );
        }
    }
    ...
    function setupNewToken(
        ...
        returns (uint256)
    {
        uint256 tokenId = _setupNewTokenAndPermission(
            newURI,
            maxSupply,
            msg.sender,
            PERMISSION_BIT_ADMIN
    ...
    function _setupNewTokenAndPermission(
    ...
    ) external onlyAdminOrRole(0, PERMISSION_BIT_METADATA) {
    ...
    function setOwner(address newOwner) external onlyAdmin(CONTRACT_BASE_ID) {
        if (
            !_hasAnyPermission(CONTRACT_BASE_ID, newOwner, PERMISSION_BIT_ADMIN)
        ) {
    ...
    function adminMintBatch(
        ...
        bool isGlobalAdminOrMinter = _isAdminOrRole(
            msg.sender,
            CONTRACT_BASE_ID,
    ...
    function _executeCommands(
        ...
        for (uint256 i = 0; i < commands.length; ++i) {
            ...
            if (method == ICreatorCommands.CreatorActions.SEND_ETH) {
                ...
                (address recipient, uint256 mintTokenId, uint256 quantity) = abi
                    .decode(commands[i].args, (address, uint256, uint256));
    ...
    function callRenderer(
        ...
        (bool success, bytes memory why) = address(getCustomRenderer(tokenId))
            .call(data);
        if (!success) {
            revert CallFailed(why);
        }
        ...
        returns (bool)
    {
        return
            ...
            interfaceId ==
            type(IHasSupportedPremintSignatureVersions).interfaceId ||
            interfaceId ==
            type(ISupportsAABasedDelegatedTokenCreation).interfaceId ||
    ...
    ) external onlyAdmin(CONTRACT_BASE_ID) {
        if (address(transferHook) != address(0)) {
            if (
                !transferHook.supportsInterface(
                    type(ITransferHookReceiver).interfaceId
                )
            ) {
    ...
    function _beforeTokenTransfer(
        ...
        if (address(config.transferHook) != address(0)) {
            config.transferHook.onTokenTransfer(
                address(this),
                operator,
                from,
                to,
    ...
    ) external onlyAdminOrRole(CONTRACT_BASE_ID, PERMISSION_BIT_FUNDS_MANAGER) {
    ...
    function updateCreateReferral(uint256 tokenId, address recipient) external {
        if (msg.sender != createReferrals[tokenId])
            revert ONLY_CREATE_REFERRAL();
    ...
    function _authorizeUpgrade(
        address _newImpl
    ) internal view override onlyAdmin(CONTRACT_BASE_ID) {
        if (
            !upgradeGate.isRegisteredUpgradePath(_getImplementation(), _newImpl)
        ) {
            revert();
        }
    }
    ...
    function supportedPremintSignatureVersions()
        external
        pure
        returns (string[] memory)
    {
    ...
    function delegateSetupNewToken(
        ...
        (
            ...
            DecodedCreatorAttribution memory attribution,
            bytes[] memory tokenSetupActions
        ) = DelegatedTokenCreation.decodeAndRecoverDelegatedTokenSetup(
                premintConfig,
                premintVersion,
                signature,
                address(this),
        ...
        emit CreatorAttribution(
            attribution.structHash,
            attribution.domainName,
            attribution.version,
            attribution.creator,
    ...
    function _delegateSetupNewToken(
        ...
        newTokenId = _setupNewTokenAndPermission(
            params.tokenURI,
            params.maxSupply,
            msg.sender,
            PERMISSION_BIT_ADMIN
