...
contract ZoraCreatorMerkleMinterStrategy is
    Enjoy,
    SaleStrategy,
    ...
    IMinterErrors
{
    ...
    event SaleSet(
        ...
        MerkleSaleSettings merkleSaleSettings
    );

    /// @notice Storage for allowed merkle settings for the sales configuration
    mapping(address => mapping(uint256 => MerkleSaleSettings))
        public allowedMerkles;
    ...
    function requestMint(
        ...
        if (
            !MerkleProof.verify(
                merkleProof,
                config.merkleRoot,
                keccak256(abi.encode(mintTo, maxQuantity, pricePerToken))
            )
        ) {
    ...
    function setSale(
        uint256 tokenId,
        MerkleSaleSettings memory merkleSaleSettings
    ) external {
    ...
    function sale(
        ...
        uint256 tokenId
    ) external view returns (MerkleSaleSettings memory) {
