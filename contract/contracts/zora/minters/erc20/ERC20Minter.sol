...
contract ERC20Minter is ReentrancyGuard, IERC20Minter, SaleStrategy, LimitedMintPerAddress, ERC20MinterRewards, Ownable {
    ...
    modifier onlyTrustedForwarder() {
        require(trustedForwarders[msg.sender], "ERC20Minter: caller is not a trusted forwarder");
        _;
    }

    constructor(address _zoraRewardRecipientAddress) Ownable() {
    ...
    function _handleIncomingTransfer(address mintTo, address currency, uint256 totalValue) internal {
        uint256 beforeBalance = IERC20(currency).balanceOf(address(this));
        IERC20(currency).safeTransferFrom(mintTo, address(this), totalValue);
    ...
    function mint(
        ...
        _handleIncomingTransfer(mintTo, currency, totalValue);
    ...
    function setTrustedForwarder(address forwarder, bool isTrusted) external onlyOwner {
