...
describe("Hat", () => {
  ...
  before(async () => {
    ...
    tx = await Gasha.connect(operator).spin(10, user.address, {
  ...
  it("should burn and redeem lost nft", async () => {
    ...
    const userHatERC721BalanceAfter = await Hat.erc721BalanceOf(user.address)
    const userLostNFTBalanceAfter = await LostNFTERC1155.balanceOf(
      user.address,
      1
    )
    ...
    expect(Number(userLostNFTBalanceAfter)).to.equal(
      Number(userLostNFTBalance) + 1
    )
  })
})
